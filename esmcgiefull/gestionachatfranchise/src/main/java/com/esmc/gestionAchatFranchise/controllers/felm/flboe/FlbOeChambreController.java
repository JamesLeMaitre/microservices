...
@RestController
@RequestMapping(value="/api/flb/oe/chambre")
public class FlbOeChambreController extends DataFormatter<FlbOeChambre> {
    @Autowired
    private FlbOeChambreInterface commonInterface;

    @PostMapping("add")
    public Object create(@RequestBody() FlbOeChambre data){
        try {
    ...
    public Object update(@PathVariable Long id, @RequestBody FlbOeChambre data) {
        try {
    ...
    public Object enable(@PathVariable Long id, @RequestBody FlbOeChambre data) {
        try {
            if( commonInterface.getById(id)==null){
                return  renderStringData(false,"Error while procssing" ,"item not found");
            }
    ...
    public Object disable(@PathVariable Long id, @RequestBody FlbOeChambre data) {
        try {
            if( commonInterface.getById(id)==null){
                return  renderStringData(false,"Error while procssing" ,"item not found");
            }
    ...
    public Object List(){
        try {
            List<FlbOeChambre> items = commonInterface.getAll();
    ...
    public Object getById(@PathVariable("id") Long id){
        try {
            FlbOeChambre item = commonInterface.getById(id);
            if(item == null){
