...
@RestController
@RequestMapping(value="/api/fill/chaineValeur")
public class FillChaineValeurController  extends DataFormatter<FillChaineValeur> {
    @Autowired
    private FillChaineValeurInterface commonInterface;

    @PostMapping("add")
    public Object create(@RequestBody() FillChaineValeur data){
        try {
    ...
    public Object update(@PathVariable Long id, @RequestBody FillChaineValeur data) {
        try {
    ...
    public Object enable(@PathVariable Long id, @RequestBody FillChaineValeur data) {
        try {
            if( commonInterface.getById(id)==null){
                return  renderStringData(false,"Error while procssing" ,"item not found");
            }
    ...
    public Object disable(@PathVariable Long id, @RequestBody FillChaineValeur data) {
        try {
            if( commonInterface.getById(id)==null){
                return  renderStringData(false,"Error while procssing" ,"item not found");
            }
    ...
    public Object List(){
        try {
            List<FillChaineValeur> items = commonInterface.getAll();
    ...
    @GetMapping("by/institution/{id}")
    public Object ListByInstitutionId(@PathVariable("id") Long id){
        try {
            List<FillChaineValeur> items = commonInterface.getListByInstitutionId(id);
            return  renderDataArray(true,items,"list of element");
        } catch (Exception e) {
            StringWriter sw = new StringWriter();
            e.printStackTrace(new PrintWriter(sw));
    ...
    public Object getById(@PathVariable("id") Long id){
        try {
            FillChaineValeur item = commonInterface.getById(id);
            if(item == null){
