...
@RestController
@RequestMapping(value="/api/flb/oe/chaineDistribution")
public class FlbOeChaineDistributionController extends DataFormatter<FlbOeChaineDistribution> {
    @Autowired
    private FlbOeChaineDistributionInterface commonInterface;

    @PostMapping("add")
    public Object create(@RequestBody() FlbOeChaineDistribution data){
        try {
    ...
    public Object update(@PathVariable Long id, @RequestBody FlbOeChaineDistribution data) {
        try {
    ...
    public Object enable(@PathVariable Long id, @RequestBody FlbOeChaineDistribution data) {
        try {
            if( commonInterface.getById(id)==null){
                return  renderStringData(false,"Error while procssing" ,"item not found");
            }
    ...
    public Object disable(@PathVariable Long id, @RequestBody FlbOeChaineDistribution data) {
        try {
            if( commonInterface.getById(id)==null){
                return  renderStringData(false,"Error while procssing" ,"item not found");
            }
    ...
    public Object List(){
        try {
            List<FlbOeChaineDistribution> items = commonInterface.getAll();
    ...
    public Object getById(@PathVariable("id") Long id){
        try {
            FlbOeChaineDistribution item = commonInterface.getById(id);
            if(item == null){
    ...
    public Object ListByChaineValeurId(@PathVariable("id") Long id){
        try {
            List<FlbOeChaineDistribution> items = commonInterface.getListByChaineValeurId(id);
            return  renderDataArray(true,items,"list of element");
        } catch (Exception e) {
            StringWriter sw = new StringWriter();
            e.printStackTrace(new PrintWriter(sw));
